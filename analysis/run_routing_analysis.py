...
def run_analysis(domain, model_name=None):
    ...
    eid2token_layer5 = defaultdict(Counter)
    eid2token_layer11 = defaultdict(Counter)
    ...
    for i, input_ids in tqdm(enumerate(load_analysis_data(tokenizer, domain=domain, bs=length))):
        ...
        exp_ids_layer5 = exp_ids[:, :, 5]
        exp_ids_layer11 = exp_ids[:, :, 11]

        for id, token in enumerate(input_ids):
            experts = exp_ids_layer0[id, :]
            for e in experts:
                eid2token_layer0[e][token] += 1
            for e in exp_ids_layer5[id, :]:
                eid2token_layer5[e][token] += 1
            for e in exp_ids_layer11[id, :]:
                eid2token_layer11[e][token] += 1
    ...
    return layer_counters, crosslayer_counters, eid2token_layer0, eid2token_layer5, eid2token_layer11
